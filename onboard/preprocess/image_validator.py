...
class ImageValidator:
    ...
    def validate(self, image: np.ndarray) -> tuple:
        ...
        # 1. None 체크
        if image is None:
            print("[ImageValidator] ❌ None image detection")
            return None, False

        # 2. ndarray 타입 체크
        if not isinstance(image, np.ndarray):
            print(f"[ImageValidator] ❌ type error: {type(image)}")
            return None, False

        # 3. shape 체크 (3채널 이미지인지)
        if len(image.shape) != 3 or image.shape[2] != 3:
            print(f"[ImageValidator] ❌ shape error: {image.shape}")
            return None, False

        # 4. 크기 체크 (너무 작은 이미지 제외)
        if image.shape[0] < 10 or image.shape[1] < 10:
            print(f"[ImageValidator] ❌ size error: {image.shape}")
            return None, False

        # 5. 데이터 타입 체크
        if image.dtype not in [np.uint8, np.float32, np.float64]:
            print(f"[ImageValidator] ❌ dtype error: {image.dtype}")
            return None, False
